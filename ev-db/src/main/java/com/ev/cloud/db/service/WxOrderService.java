...
import com.ev.cloud.db.util.CouponUserConstant;
import com.ev.cloud.db.util.JsonResult;
import com.ev.cloud.db.util.OrderHandleOption;
import com.ev.cloud.db.util.OrderUtil;
import com.ev.common.base.constant.SystemConfig;
import com.ev.common.core.express.ExpressService;
import com.ev.common.core.express.dao.ExpressInfo;
import com.ev.common.utils.DateTimeUtil;
import com.ev.common.utils.IpUtil;
 ...
 * @return 订单列表
    ...
    public JsonResult list(Integer showType, Integer page, Integer limit, Integer userId) {

        List<Short> orderStatus = OrderUtil.orderStatus(showType);
        List<LitemallOrder> orderList = orderService.queryByOrderStatus(userId, orderStatus, page, limit);
        long count = new PageInfo<>(orderList).getTotal();
 ...
 * @return 订单详情
    ...
    public JsonResult detail(Integer orderId, Integer userId) {
        ...
        if (!order.getUserId().equals(userId)) {
            return JsonResult.error("不是当前用户的订单");
        }
 ...
 * @return 提交订单操作结果
    ...
    @Transactional
    public JsonResult submit(String body, Integer userId) {
        if (body == null) {
...
/*  if (grouponRules != null && grouponRules.getGoodsId().equals(checkGoods.getGoodsId())) {
        ...
        order.setMessage(checkedAddress.getAreaCode());
        String detailedAddress = checkedAddress.getProvince() + checkedAddress.getCity() + checkedAddress.getCounty() + " " + checkedAddress.getAddressDetail();
        //String detailedAddress = checkedAddress.getAreaCode() + " " + checkedAddress.getAddressDetail();
 ...
 * @return 取消订单操作结果
    ...
    @Transactional
    public JsonResult cancel(String body, Integer userId) {
        ...
        if (!handleOption.isCancel()) {
            return JsonResult.error("订单不能取消");
        }
 ...
 * @return 支付订单ID
    ...
    @Transactional
    public JsonResult prepay(String body, HttpServletRequest request, Integer userId) {
        ...
        if (!handleOption.isPay()) {
            return JsonResult.error("订单不能支付");
        }
 ...
 * @return 订单退款操作结果
    ...
    public JsonResult refund(String body, Integer userId) {
 ...
 * @return 订单操作结果
    ...
    public JsonResult confirm(String body, Integer userId) {
        ...
        if (!handleOption.isConfirm()) {
            return JsonResult.error("订单不能确认收货");
        }
 ...
 * @return 订单操作结果
    ...
    public JsonResult delete(String body, Integer userId) {
        ...
        if (!handleOption.isDelete()) {
            return JsonResult.error("订单不能删除");
        }
...
/**
    ...
    public JsonResult goods( Integer orderId, Integer goodsId) {
        ...
        return JsonResult.ok(orderGoods);
    }
 ...
 * @return 订单操作结果
    ...
    public JsonResult comment(String body, Integer userId) {
        ...
        if (!OrderUtil.isConfirmStatus(order) && !OrderUtil.isAutoConfirmStatus(order)) {
            return JsonResult.error("当前商品不能评价");
        }
        if (!order.getUserId().equals(userId)) {
            return JsonResult.error("当前商品不属于用户");
        }
        Integer commentId = orderGoods.getComment();
        if (commentId == -1) {
            return JsonResult.error("当前商品评价时间已经过期");
        }
        if (commentId != 0) {
            return JsonResult.error("订单商品已评价");
        }
